...
tfit <- coxme(Surv(dummy, time, status) ~ x1 + x2, data=tdata0,
              random= ~1|grp, variance=.5, weight=wt, iter=0,
              ties='efron', init=c(pi,0), sparse.calc=0)
aeq(tfit$loglik[3], lfun(pi,T))
aeq(tfit$u[3], ufun(pi,T))
...
tfit1 <- coxme(Surv(dummy, time, status) ~ x1 + x2, data=tdata0,
              random= ~1|grp, variance=.5, weight=wt, iter=0,
              ties='efron', init=c(pi,0), sparse.calc=1)
aeq(tfit$u, tfit1$u)
aeq(tfit$var, tfit1$var)
...
tfit1 <- coxme(Surv(time1, time2, status) ~ x1 + x2, data=tdata0b,
              random= ~1|grp, variance=.5, weight=wt, iter=0,
              ties='efron', init=c(pi,0), sparse.calc=1)
aeq(tfit$u, tfit1$u)
aeq(tfit$var, tfit1$var)
