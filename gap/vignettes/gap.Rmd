...
e.g., $r2 \le 0.5$) using expressions that rely on effect sizes and their standard
errors @graph21. More specifically, let Bayes factors (BF) for each variant in the
...
$BF_i/\sum_{i=1}^TBF_i$, where $i$ indexes all variants considered in the region. We
generated credible sets within a given region by ranking all variants by PPs in
descending order and calculating the number of variants required to reach a cumulative
