...
2. vignettes with noweb/Sweave as well as **rmarkdown** and **bookdown** that allows
   for numbered sectioning, Citation Style Language (CSL) (https://citationstyles.org/),
   and multiple figures generated from a code chunk. **Rdpack** now enables BibTeX
   citations in the .Rd files.
...
In line with the surge of interest in the polygenic models, a separate vignette is
available through `vignette("h2",package="gap.examples")` demonstrating aspect of
...
dizygotic (DZ) twin correlations under the additive genetics, and shared environment
(ACE) model, e.g., [10.1038/s41562-023-01530-y](https://doi.org/10.1038/s41562-023-01530-y).
...
mr_forestplot(tnfb, colgap.forest.left="0.05cm", fontsize=14, leftlabs=c("Outcome","b","SE"),
              ...
              spacing=1.6,digits.TE=2,digits.se=2,xlab="Effect size")
```
