...
The function `mr` was originally developed to rework on data generated from GSMR, although it could be any harmonised data.

```{r mr, fig.cap="Mendelian randomization", fig.height=7, fig.width=7}
...
mrdat <- setNames(as.data.frame(mrdat), c("SNP", "b.LIF.R", "SE.LIF.R", "b.FEV1", "SE.FEV1", "b.CAD", "SE.CAD"))
knitr::kable(mrdat,caption="Table 2. LIF.R and CAD/FEV1")
res <- gsmr(mrdat, "LIF.R", c("CAD","FEV1"),other_plots=TRUE)
f <- "INF1_CAD-FEV1.csv"
...
p <- top(r)
rp <- t(data.frame(r[-1],p))
colnames(rp) <- r[[1]]
knitr::kable(format(rp,digits=3),caption="Table 3. LIFR variant rs635634 and CAD/FEV1",align="r")
unlink(f)
```
