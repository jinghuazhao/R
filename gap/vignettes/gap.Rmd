...
Therefore they differ from the asymptotic results @kotz06 by a scale factor of $(N-2)(N+1)/(N-1)^2$ and $(N-2)/(N+1)$ for
linear regression and $t$-statistic, respectively.

```{r pve, echo=FALSE, fig.cap="Scale factors as function of sample size (N)", fig.height=6, fig.width=8}
N <- 2:500
LR <- (N-2)*(N+1)/(N-1)^2
t <- (N-2)/(N+1)
plot(N,LR,cex=0.6,xaxt="n",xlab="Sample size",ylab="Scale factor",col="red",pch=19)
points(N,t,cex=0.6,pch=20,col="blue")
abline(h=1,col="black")
axis(1,at=c(2,(1:5)*100))
legend(400,0.5,c("LR","t-statistic"),col=c("red","blue"),pch=c(19,20))
```

It turns out that as the sample size increases, they approach 1 quickly from opposite directions after $N\approx 300$.
