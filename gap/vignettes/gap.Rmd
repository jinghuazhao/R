...
When z is very large, its corresponding p value is very small. A genomewide significance is declared at 0.05/1000000=5e-8 with Bonferroni correction assuming 1 million SNPs are tested. This short note describes how to get -log10(p), which can be used in a Q-Q plot and software such as DEPICT. The solution here is generic since z is also the square root of a chi-squared statistic, for instance.
...
I believe by now the package should have given you a flavor of initiatives that
I have made so far in relation to how the project was envisaged. More importantly,
it is clear that availability of the package will serve as a infrastructure with
which future work can be accumulated and collaboration can be built. More likely,
it will inspire others to join the efforts to develop their own packages.
