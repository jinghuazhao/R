...
knitr::kable(list(table1,alz),caption="Table 1. Power/Sample size of family-based designs (Alzheimer's disease)")
```
...
knitr::kable(table5,caption="Table 2. Sample size of population-based design")
```
...
knitr::kable(read.table(outfile,header=TRUE,sep="\t"),caption="Table 3. Sample size of case-cohort designs")
unlink(outfile)
```
...
We have the R function `z <- function(p) qnorm(p/2,lower.tail=FALSE)`.
...
knitr::kable(v,caption="Table 4. z,P,log(P) and log10(P)")
```
...
The function `mr` was originally developed to rework on data generated from GSMR, although it could be any harmonised data. The following example is from analysis of a real data on LIF-R protein and CVD/FEV1.
...
knitr::kable(mrdat,caption="Table 5. LIF-R and CAD/FEV1")
...
Lastly, we illustrate MR results in forest plots with `mr_forestplot()`.
...
```{r mr_forestplot, fig.cap="Forest plots based on MR results on TNFB", fig.align="left", fig.height=6, fig.width=9, results="hide"}
...
future work can be accumulated and collaboration can be built. More likely, it
will inspire others to join the efforts to develop their own packages.
