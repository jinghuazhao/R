...
METAL_forestplot(OPGtbl[4:6,],OPGall,OPGrsid,width=6.75,height=5,digits.TE=2,digits.se=2)
```
...
```{r pve, echo=FALSE, fig.cap="Scale factors as function of sample size (N)", fig.height=6, fig.width=14, messages=FALSE, warning=FALSE}
oldpar <- par()
par(mfrow=c(1,2))
N <- 2:500
R2 <- 2*(N-2)/(N-1)^2/(N+1)
...
As the sample size increases, the estimates are quite close nevertheless quite small while the ratios approach 1 quickly from opposite directions after $N\approx 300$.
