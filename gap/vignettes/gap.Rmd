...
immediate benefit is something complementary to other packages currently
available from CRAN, e.g. **genetics**, **hwde**, etc. I hope eventually this will
be part of a bigger effort to fulfil most of the requirements foreseen by many @guo00,
...
The number of functions are quite limited and experimental, but I feel the
enormous advantage by shifting to R and would like to my work with others as it is
available, which I will not claim this work as exclusively done by myself,
...
METAL @willer10 and `sentinels` which extracts sentinels from GWAS summary statistics
...
Recent technical updates are worth mentioning:

1. Documentaion via **roxygen2** from R code to allow for easier generation of
   the .Rd files and NAMESPACE through `devtools::document()`

2. vignettes through noweb/Sweave as well as **rmarkdown** and **bookdown** which
   format references in Citation Style Language (CSL) (https://citationstyles.org/),
   and embed multiple figures generated from a code chunk. **Rdpack** now unifies
   BibTeX citations throughout the package though CSL is not yet fully supported.

3. an experimental shiny App (`runshinygap`()).

These experiences will hopefully be shared with/useful for other colleagues
...
of the genomewide association studies (GWAS) therefore are likely to be more familiar than
their R couunterparts in `gap`. However, you need to follow their instructions to compile
...
`ms` code @hudson02, which is required by `read.ms.output`.

A final note is concerned about `twinan90`, which is now dropped from the package function
list due to difficulty to keep up with the requirements by the `R` environment but
...
Functions `h2G`, `h2GE` and `h2l` are briefly documented. Functions `h2.jags` and `hwe.jags` are also available.
