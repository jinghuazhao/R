...
2. vignettes with noweb/Sweave as well as **rmarkdown** and **bookdown** that allows
   or Citation Style Language (CSL) (https://citationstyles.org/), and multiple
   figures generated from a code chunk. **Rdpack** now enables BibTeX citations in
   the .Rd files where CSL is not yet supported.
...
occasionally useful to use specific functions without loading the whole package,
i.e., `library(gap)`: e.g.,`invnormal <- gap::invnormal`, `log10p <- gap::log10p`.
