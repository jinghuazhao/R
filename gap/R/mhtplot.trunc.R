                           ...
                           mtext.line=2, cex.y= 1, y.ax.space=5, y.brk1, y.brk2, ...) 
{
  ...
  if (nchr == 1) axis(1, ...)
  else axis(1, at = ticks, labels = labs, ...)
  ...
  if (!is.null(annotatePval)) {
    ...
    par(xpd = TRUE)
    if (annotateTop == FALSE)  with(subset(d, P <= annotatePval), calibrate::textxy(pos, -log10p(P), 
      ...
      calibrate::textxy(topSNPs$pos, -log10p(topSNPs$P), offset = 0.625, labs = topSNPs$SNP, cex = cex.text, ...)
    }
  }
  par(xpd = FALSE)
}
